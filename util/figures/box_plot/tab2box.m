...
% 
%   Y = TAB2BOX(XIN,YIN) prepares data, in tabular form, for use in the
%   BOX_PLOT function. Specifically, XIN and YIN are vectors (for example
%   column vectors from a results table). Y is an N-by-P numeric array,
%   where P is the number of unique elements in XIN, and N is the maximum
%   number of occurences of any individual element of XIN. In cases where
%   elements in XIN do not occur an equal number of times, columns in YIN
%   are padded with NaNs.
% 
%   Y = TAB2BOX(XIN,YIN,GIN) returns an N-by-P-by-G-by-I-by-J... numeric
%   array, where G is the number of unique elements in the first column of
%   GIN, I is the number of unique elements in the second column of GIN,
%   etc. GIN is a numeric or cell matrix with as many rows as XIN or YIN.
%   The input facilitates hierarchical grouping of the data in the box
%   plot, with the grouping order determined by the column order of GIN.
% 
%   [Y,X] = TAB2BOX(...) returns the unique values in XIN to X.
% 
%   [Y,X,G] = TAB2BOX(...) returns the unique values in GIN to G. G is a
%   cell vector whereby the Nth element contains a vector of length
%   SIZE(Y,N+2).
% 
%   Example
% 
...
% 
%       % arrange data
%       [y,x,g] = tab2box(Cylinders,MPG,when);
%   
%       % sort
%       IX = [1 3 2]; % order
%       g = g{1}(IX);
...
%           'scalewidth',true,'boxwidth',.25,'xseparator',true,...
%           'groupwidth',.6);
%       box on
%       legend(squeeze(h.boxes(:,1,:)),g,'location','best')
...
% =========================================================================
% Last changed:     $Date: 2015-10-13 14:57:10 +0100 (Tue, 13 Oct 2015) $
% Last committed:   $Revision: 431 $
